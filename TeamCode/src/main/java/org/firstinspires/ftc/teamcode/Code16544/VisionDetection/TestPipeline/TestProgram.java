...
import static org.firstinspires.ftc.teamcode.Code16544.VisionDetection.TestPipeline.test.Color.RED;
...
public class TestProgram extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        test detector = new test(telemetry);
        ...
        while (opModeIsActive()) {
            while (detector.getLocation() == test.Location.LEFT) {
                telemetry.addData("ELEMENT", "LEFT");
                telemetry.update();

            }
            while (detector.getLocation() == test.Location.RIGHT) {
                telemetry.addData("ELEMENT", "RIGHT");
                telemetry.update();
            }
            while (detector.getLocation() == test.Location.CENTRE) {
